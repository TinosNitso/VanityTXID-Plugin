...
A fundamental issue is that 0-conf doesn't apply to the TXID itself. The payment amount can't be changed, but the TXID & message can change before confirmation. If it ever fails, the contract can be improved. eg a new version can include simple nonce checksum/s, s.t. miners can't ever deduce a vanity contract is being used ("hacks" can go either way, and mining pools might demand higher fees for vanity contracts). A smarter contract could also sign 0-conf message using OP_CHECKDATASIG.
...
- ~31% speed increase by using Bitcoin Core's CSHA256 C++ code. Binary sizes are all much larger now. I've removed the zedwood license. In a future update I might bring it back as a UI selection, since zedwood probably wins on simplicity (imagine having to write every line yourself). Users might want to select between CryptoPP, OpenSSL, Bitcoin Core & zedwood, to check the hash rates. I've noticed results which are a bit too impressive inside Linux & macOS VMs. eg I got 1.6 MH/s using only 4 Threads in a very slow macOS Catalina VirtualBox (in screenshot, running in background). This should mean I get more than (1.8->1.9) MH/s natively in Windows. I'll probably try a native Linux test. It could be there's an issue with MinGW being slow (poor Windows build).
