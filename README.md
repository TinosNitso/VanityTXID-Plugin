...
In the next update the hashrate timing will be a tiny bit better, include the 8th Nonce byte, and the formula should use (Nonce+1), not Nonce. One issue with controlling TTS speed is Windows might have a different UI to posix, since they specify words-per-minute instead of -10->10. Maybe pronouncing a few more digits would be better.
