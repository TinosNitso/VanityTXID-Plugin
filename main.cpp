...
void Hasher(int ThreadN,char **argv) {
    ...
    std::string OddHex;
    if (PatternOdd) {
        OddHex+=Pattern[PatternLen-1];
        OddHex=FromHex(OddHex);
    }
    int P2=P1+1;
    int P3=P2+1;
    int P4=P3+1;
    ...
    for (String[P8]=0;String[P8]!=-1;String[P8]++) {
        ...
        if (PatternOdd)
            Finished=Finished and OddHex[0]==(unsigned char)sha256d[31-PatternSize]/16;
        if (Finished) break;
    }   if (Finished) break;
    }   if (Finished) break;
    }   if (Finished) break;
    ...
    char buf[2*String.length()];
    for (int i = 0; i < String.length(); i++)
        sprintf(buf+2*i, "%02x", (unsigned char) String[i]);
    cout<<buf<<std::flush;
